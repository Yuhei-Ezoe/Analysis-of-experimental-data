...
columns_list_sorted.insert(0, "Wavelength_nm")
columns_list_sorted.insert(1, "Photon Energy_eV")
...
while m < len(columns_list_sorted):
    ...
    #グラフにプロット
    ax.plot(df["Photon Energy_eV"], df[columns_list_sorted].iloc[:, m], label="F="+str(columns_list_sorted[m])+"Hz")
    ...
    #軸ラベル
    ax.set_xlabel("Phoon Energy (eV)", fontsize = 15)
    ax.set_ylabel("Response (A/W)", fontsize = 15)
